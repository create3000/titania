...
class X3DArrayField :
	public X3D::X3DField <X3D::Array <ValueType>>
{
public:

	typedef ValueType         scalar_type;
	typedef X3D::Array <ValueType> value_type;
	...
	using X3D::X3DField <value_type>::getValue;
	using X3D::X3DField <value_type>::set;
	using X3D::X3DField <value_type>::operator =;

	///  Default constructor.
	X3DArrayField () :
		X3D::X3DField <value_type> ()
	{ }

	///  Copy constructor.
	X3DArrayField (const X3DArrayField & field) :
		X3D::X3DField <value_type> (field)
	{
	...
	X3DArrayField (const ValueType (&value) [Size]) :
		X3D::X3DField <value_type> (std::move (value_type (value, value + Size)))
	{
		addChildren (begin (), end ());
	}
	...
	X3DArrayField (std::initializer_list <ValueType> initializer_list) :
		X3D::X3DField <value_type> (std::move (value_type (initializer_list .begin (), initializer_list .end ())))
	{
		addChildren (begin (), end ());
	}
	...
	X3DArrayField (InputIterator first, InputIterator last) :
		X3D::X3DField <value_type> (std::move (value_type (first, last)))
	{
		addChildren (begin (), end ());
	}
	...
	virtual
	X3DArrayField*
	clone (X3D::X3DExecutionContext* const) const { return clone (); }
...
protected:
	...
	using X3D::X3DField <value_type>::get;


private:

	using X3D::X3DField <value_type>::notifyParents;
...
typedef X3DArrayField <X3D::SFString>    MFString;
...
main (int argc, char* argv [ ])
{
	...
	std::clog << "Test started ..." << std::endl << std::endl;
	{
		...
		{
			X3D::MFString string1 ({ "" });
			X3D::MFString s ({ "level1.wrl" });
			
			string1 .set (s);
			std::clog << string1 << std::endl;
			
			
			X3D::MFFloat bools ({ false });
			X3D::MFFloat b ({ true });
			
			bools .set (b);
			std::clog << bools << std::endl;
		}
