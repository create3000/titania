...
#ifndef __TITANIA_INPUT_OUTPUT_INVERSE_CHARACTER_H__
#define __TITANIA_INPUT_OUTPUT_INVERSE_CHARACTER_H__
...
class basic_inverse_character
{
public:

	///  @name Construction

	///  Constructs the io::basic_inverse_character from @a string.  A std::basic_istream can then be tested against the string.
	constexpr
	basic_inverse_character (const CharT character) :
		character (character)
	{ }
...
private:
	...
	const io::basic_character <CharT, Traits> character;

};

template <class CharT, class Traits>
bool
basic_inverse_character <CharT, Traits>::operator () (std::basic_istream <CharT, Traits> & istream, std::basic_string <CharT> & string) const
{
	if (istream)
	{
		do
		{
			if (character (istream))
				return true;
