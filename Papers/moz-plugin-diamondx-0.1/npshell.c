...
printf("NPP_New()\n");
    ...
    for (i = 0; i < argc; i++)
    {
        ...
        if (!strcmp(argn[i], "wmode") && !strcmp(argv[i], "transparent"))
        {
            /* no window */
...
printf("NPP_Destroy()\n");
    ...
    if (This != NULL)
    {
        ...
        {
            if (This->ximage->data)
                NPN_MemFree(This->ximage->data);
            NPN_MemFree(This->ximage);
        }
        if (This->four_quads)
            NPN_MemFree(This->four_quads);
...
printf("NPP_SetWindow()\n");
    ...
    return DiamondXSetWindow(instance, window);
}
