...
/* $Id: //mis/devmarketing/projects/devzone/release/devzone-0.9.1/sites/all/modules/custom/nvidia_quick_survey/nvidia_quick_survey.js#1 $ */

$(document).ready(function() {
  $('[class*="quicksurvey-"]').attr('onClick', 'quickSurveyExecute(this)');
});
...
function quickSurveyExecute(e) {
  ...
  $("#quicksurvey-output").remove();

  var outputHolder = $("<div id='quicksurvey-output'></div>");
  ...
  surveyDelay = Drupal.settings.nvidia_quick_survey_settings.surveyDelay;
  setTimeout(function() {
    // .get will not execute the function unless it's successful
    $.get(survey_url, null, function(data){
      ...
      $('#quicksurvey-output').parent().attr("id", "quicksurvey-output-wrapper");
      ...
      $('#quicksurvey-output-wrapper').css('top', topPosition);

      // Keep our modal centered if user resizes browser
      $(window).resize(function() {
        leftMargin = ($(window).width() - $('#quicksurvey-output-wrapper').width()) / 2;
        $('#quicksurvey-output-wrapper').css('left', leftMargin);
      });
    }, 'json');
  }, surveyDelay);
 ...
 */
Drupal.behaviors.CustomSearch = function(context) {
	$("#edit-search-theme-form-1").val("Search DevZone");
	/*$("#edit-keys").val("Search Resources");*/
	/*$("#edit-mail").val("Enter Email Address");*/

	$("#edit-search-theme-form-1,#edit-keys,#edit-mail").css('color', '#9E9E9E')
	.focus(function () {
		if($(this).attr('status') != 'searched') {
			$(this).val("").css('color', 'black').attr('status', 'searched');
		}
    });
}
;
