	...
	if (x3d_Lighting)
	{
		...
		for (int i = 0; i < x3d_MaxLights; ++ i)
		{
			...
			if (di || dL <= x3d_LightRadius [i])
			{
				...
				float shadowFactor                = 1.0 - getShadowIntensity (i, lightType, x3d_ShadowIntensity [i], x3d_ShadowDiffusion [i], x3d_ShadowMatrix [i], lightAngle);

				finalColor += (shadowFactor * attenuationSpotFactor) * (x3d_LightColor [i] * ambientDiffuseSpecularColor);
			}
		}
