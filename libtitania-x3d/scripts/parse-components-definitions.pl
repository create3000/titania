...
sub create_h
{
	...
	open FILE, ">", "$component/$node->{typeName}.h";
	print FILE <<EOT;
...
#include "../Core/$node->{base}.h"

namespace titania {
namespace X3D {

class $node->{typeName} :
	virtual public $node->{base}
{
public:
EOT
...
sub create_cpp
{
	...
	open FILE, ">", "$component/$node->{typeName}.cpp";
	print FILE <<EOT;
/* -*- Mode: C++; coding: utf-8; tab-width: 3; indent-tabs-mode: tab; c-basic-offset: 3 -*-
 *******************************************************************************
 *
...
sub is_in_base
{
	...
	my $base = get_node ($node -> {base});
	
	return 0 unless $base;
	
	foreach my $f (@{$base -> {fields}})
	{
		return 1 if $f -> {name} eq $field -> {name};
	}
	
	return is_in_base ($base, $field);
}
