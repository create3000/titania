...
const Component   TextureCoordinate::component      = Component ("Texturing", 1);
...
Vector4f
TextureCoordinate::get1Point (const size_t index) const
{
	if (index < point () .size ())
	{
...
TextureCoordinate::addTexCoord (const size_t i, MultiTexCoordArray & texCoords, const size_t index) const
{
	...
	{
		const Vector2f & point2 = point () [index % point () .size ()];
		texCoords [i] .emplace_back (point2 .x (), point2 .y (), 0, 1);
	}
	else
