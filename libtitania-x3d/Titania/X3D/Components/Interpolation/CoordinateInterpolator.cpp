...
CoordinateInterpolator::initialize ()
{
	...
	keyValue .addInterest (this, &CoordinateInterpolator::set_keyValue);
}
...
void
CoordinateInterpolator::interpolate (size_t index0, size_t index1, float weight)
{
	...
	{
		value_changed [i] = math::lerp <Vector3f> (keyValue [index0 + i],
		                                           keyValue [index1 + i],
		                                           weight);
	}
}
