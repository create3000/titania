...
JSBool
jsX3DProtoDeclaration::create (JSContext* const context, const X3DPtr <Proto> & proto, jsval* const vp)
{
	...
	const auto field = new X3DPtr <Proto> (proto);
...
{
	const auto & proto = *static_cast <X3DPtr <Proto>*> (JS_GetPrivate (context, obj));
...
{
	auto & proto = *static_cast <X3DPtr <Proto>*> (JS_GetPrivate (context, obj));
...
{
	const auto & proto = *static_cast <X3DPtr <Proto>*> (JS_GetPrivate (context, JS_THIS_OBJECT (context, vp)));

	X3DPtr <X3DPrototypeInstance> instance = proto -> createInstance ();
