...
JSPropertySpec SFColor::properties [ ] = {
...
JSObject*
SFColor::init (JSContext* const cx, JSObject* const global, JSObject* const parent)
{
...
JSBool
SFColor::construct (JSContext* cx, uint32_t argc, jsval* vp)
{
	try
	{
		switch (argc)
		{
			case 0:
			{
				return create <SFColor> (cx, new X3D::SFColor (), &JS_RVAL (cx, vp));
			}
			case 3:
			{
...
JSBool
SFColor::enumerate (JSContext* cx, JSObject* obj, JSIterateOp enum_op, jsval* statep, jsid* idp)
{
...
JSBool
SFColor::set1Value (JSContext* cx, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	try
	{
...
JSBool
SFColor::get1Value (JSContext* cx, JSObject* obj, jsid id, jsval* vp)
{
	try
	{
		const auto lhs = getThis <SFColor> (cx, obj);
...
JSBool
SFColor::getHSV (JSContext* cx, uint32_t argc, jsval* vp)
{
	if (argc not_eq 0)
...
JSBool
SFColor::setHSV (JSContext* cx, uint32_t argc, jsval* vp)
{
	if (argc not_eq 3)
