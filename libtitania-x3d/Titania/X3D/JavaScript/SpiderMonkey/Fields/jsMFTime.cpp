...
template <>
JSBool
jsX3DArrayField <jsSFTime, X3D::MFTime>::construct (JSContext* cx, uint32_t argc, jsval* vp)
{
	try
	{
		if (argc == 0)
		{
			return create (cx, new X3D::MFTime (), &JS_RVAL (cx, vp));
		}
		else
		{
			const auto array = new X3D::MFTime ();
...
template <>
JSBool
jsX3DArrayField <jsSFTime, X3D::MFTime>::set1Value (JSContext* cx, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	try
	{
		...
		const auto array = getThis <jsX3DArrayField <jsSFTime, X3D::MFTime>> (cx, obj);
...
template <>
JSBool
jsX3DArrayField <jsSFTime, X3D::MFTime>::unshift (JSContext* cx, uint32_t argc, jsval* vp)
{
	if (argc not_eq 1)
	...
	{
		const auto argv  = JS_ARGV (cx, vp);
		const auto array = getThis <jsX3DArrayField <jsSFTime, X3D::MFTime>> (cx, vp);
...
template <>
JSBool
jsX3DArrayField <jsSFTime, X3D::MFTime>::push (JSContext* cx, uint32_t argc, jsval* vp)
{
	if (argc not_eq 1)
