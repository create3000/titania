...
jsMFTime::get1Value (JSContext* context, JSObject* obj, jsid id, jsval* vp)
{
	...
	return JS_NewNumberValue (context, mftime -> get1Value (index), vp);
}

JSBool
jsMFTime::set1Value (JSContext* context, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
	...
	mftime -> set1Value (index, SFTime (number));
