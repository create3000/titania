...
jsMFVec3d::construct (JSContext* context, uintN argc, jsval* vp)
{
	...
	else
	{
		...
		for (uintN i = 0; i < argc; ++ i)
		{
			...
			{
				JS_ReportError (context, "Type of argument %d is invalid - should be SFVec3d, is %s", i, JS_GetClass (context, value) -> name);
				return JS_FALSE;
			}

			values [i] = *(SFVec3d*) JS_GetPrivate (context, value);
		}
...
jsMFVec3d::get1Value (JSContext* context, JSObject* obj, jsid id, jsval* vp)
{
	...
	return jsSFVec3d::create (context, (SFVec3d*) &field -> get1Value (index), vp);
}
...
jsMFVec3d::set1Value (JSContext* context, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	...
	field -> set1Value (index, *(SFVec3d*) JS_GetPrivate (context, value));
