...
public:
	...
	typedef typename jsSFVec2 <typename Type::vector2_type>::value_type vector2_value_type;
	typedef typename jsSFRotation::value_type                           rotation_value_type;
...
jsSFMatrix3 <Type>::setTransform (JSContext* context, uintN argc, jsval* vp)
{
	if (argc >= 0 or argc <= 5)
	{
		...
		if (argc >= 1)
		{
			...
			vector2_value_type* translation = (vector2_value_type*) JS_GetPrivate (context, translationObj);
			...
			if (argc >= 2)
			{
				...
				//rotation_value_type* rotation = (rotation_value_type*) JS_GetPrivate (context, rotationObj);
				...
				if (argc >= 3)
				{
					...
					//vector2_value_type scale = (vector2_value_type*) JS_GetPrivate (context, scaleObj);
					...
					if (argc >= 4)
					{
						...
						//rotation_value_type* scaleOrientation = (rotation_value_type*) JS_GetPrivate (context, scaleOrientationObj);
						...
						if (argc >= 5)
						{
							...
							//vector2_value_type* center = (vector2_value_type*) JS_GetPrivate (context, centerObj);
...
jsSFMatrix3 <Type>::multVecMatrix (JSContext* context, uintN argc, jsval* vp)
{
	if (argc == 1)
	{
		...
		vector2_value_type* vector = (vector2_value_type*) JS_GetPrivate (context, rhs);
