...
template <>
JSBool
MFNode::construct (JSContext* cx, uint32_t argc, jsval* vp)
{
	try
	{
		if (argc == 0)
		{
			return create <MFNode> (cx, new X3D::MFNode (), &JS_RVAL (cx, vp));
		}
		else
		{
			...
			return create <MFNode> (cx, array, &JS_RVAL (cx, vp));
		}
	}
...
template <>
JSBool
MFNode::set1Value (JSContext* cx, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	try
	{
