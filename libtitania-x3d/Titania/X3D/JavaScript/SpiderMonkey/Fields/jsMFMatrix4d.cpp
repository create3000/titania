...
JSBool
jsMFMatrix4d::construct (JSContext* context, uintN argc, jsval* vp)
{
	...
	else
	{
		...
		JS_SetPrivate (context, obj, new MFMatrix4d (values, values + argc));
...
JSBool
jsMFMatrix4d::set1Value (JSContext* context, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
