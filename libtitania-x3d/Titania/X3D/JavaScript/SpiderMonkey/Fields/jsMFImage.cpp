...
jsMFImage::construct (JSContext* context, uintN argc, jsval* vp)
{
	...
	else
	{
		...
		for (uintN i = 0; i < argc; ++ i)
		{
			...
			{
				JS_ReportError (context, "Type of argument %d is invalid - should be SFImage, is %s", i, JS_GetClass (context, value) -> name);
				return JS_FALSE;
			}

			values [i] = *(SFImage*) JS_GetPrivate (context, value);
		}
...
jsMFImage::get1Value (JSContext* context, JSObject* obj, jsid id, jsval* vp)
{
	...
	return jsSFImage::create (context, (SFImage*) &field -> get1Value (index), vp);
}
...
jsMFImage::set1Value (JSContext* context, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	...
	field -> set1Value (index, *(SFImage*) JS_GetPrivate (context, value));
