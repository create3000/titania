...
jsMFVec3f::construct (JSContext* context, uintN argc, jsval* vp)
{
	...
	else
	{
		...
		for (uintN i = 0; i < argc; ++ i)
		{
			...
			{
				JS_ReportError (context, "Type of argument %d is invalid - should be SFVec3f, is %s", i, JS_GetClass (context, value) -> name);
				return JS_FALSE;
			}

			values [i] = *(SFVec3f*) JS_GetPrivate (context, value);
		}
...
jsMFVec3f::get1Value (JSContext* context, JSObject* obj, jsid id, jsval* vp)
{
	...
	return jsSFVec3f::create (context, (SFVec3f*) &field -> get1Value (index), vp);
}
...
jsMFVec3f::set1Value (JSContext* context, JSObject* obj, jsid id, JSBool strict, jsval* vp)
{
	...
	field -> set1Value (index, *(SFVec3f*) JS_GetPrivate (context, value));
