...
JSBool
jsMFVec3f::construct (JSContext* context, uintN argc, jsval* vp)
{
	if (argc == 0)
	{
		return create (context, new MFVec3f (), &JS_RVAL (context, vp));
	}
	else
	{
		SFVec3f values [argc];
		...
		return create (context, new MFVec3f (values, values + argc), &JS_RVAL (context, vp));
	}
