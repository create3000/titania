...
JSBool
jsMFColor::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
		JS_SetPrivate (context, obj, new MFColor ());
		return JS_TRUE;
	}
	else
	{
		...
		for (MFColor::size_type i = 0; i < argc; ++ i)
		{
			JSObject* obj2;
		...
		JS_SetPrivate (context, obj, new MFColor (values, argc));
...
JSBool
jsMFColor::enumerate (JSContext* context, JSObject* obj, JSIterateOp enum_op, jsval* statep, jsid* idp)
{
	MFColor* x3darrayfield = (MFColor*) JS_GetPrivate (context, obj);
...
JSBool
jsMFColor::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
...
JSBool
jsMFColor::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
