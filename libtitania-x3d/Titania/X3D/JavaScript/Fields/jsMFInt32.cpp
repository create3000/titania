...
JSBool
jsMFInt32::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
	...
	else
	{
		...
		for (MFInt32::size_type i = 0; i < argc; ++ i)
		{
			int32 number;
		...
		JS_SetPrivate (context, obj, new MFInt32 (values, argc));
...
JSBool
jsMFInt32::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	...
	return JS_NewDoubleValue (context, mfint32 -> get1Value (index), vp);
}

JSBool
jsMFInt32::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
	...
	mfint32 -> set1Value (index, number);
