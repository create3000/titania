...
JSBool
jsMFNode::create (JSContext* context, MFNode* field, jsval* vp, const bool seal)
{
...
JSBool
jsMFNode::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
		JS_SetPrivate (context, obj, new MFNode ());
		return JS_TRUE;
	}
	else
	{
		...
		for (MFNode::size_type i = 0; i < argc; ++ i)
		{
			JSObject* obj2;
			...
			values [i] = *(SFNode*) JS_GetPrivate (context, obj2);
		}

		JS_SetPrivate (context, obj, new MFNode (values, argc));
...
{
	MFNode* x3darrayfield = (MFNode*) JS_GetPrivate (context, obj);
...
JSBool
jsMFNode::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
...
JSBool
jsMFNode::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
	...
	mfnode -> set1Value (index, *(SFNode*) JS_GetPrivate (context, obj2));
