...
JSClass jsMFFloat::static_class = {
...
void
jsMFFloat::init (JSContext* context, JSObject* global)
{
...
JSBool
jsMFFloat::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
	...
	else
	{
		...
		for (MFFloat::size_type i = 0; i < argc; ++ i)
		{
			jsdouble number;
		...
		JS_SetPrivate (context, obj, new MFFloat (values, argc));
...
JSBool
jsMFFloat::enumerate (JSContext* context, JSObject* obj, JSIterateOp enum_op, jsval* statep, jsid* idp)
{
	MFFloat* x3darrayfield = (MFFloat*) JS_GetPrivate (context, obj);
...
JSBool
jsMFFloat::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	...
	return JS_NewDoubleValue (context, mffloat -> get1Value (index), vp);
}

JSBool
jsMFFloat::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
	...
	mffloat -> set1Value (index, number);
