...
JSBool
jsMFString::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
	...
	else
	{
		...
		for (MFString::size_type i = 0; i < argc; ++ i)
		{
			values [i] = std::string (JS_GetStringBytes (JS_ValueToString (context, argv [i])));
		}

		JS_SetPrivate (context, obj, new MFString (values, argc));
...
JSBool
jsMFString::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
...
JSBool
jsMFString::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
	...
	mfstring -> set1Value (index, std::string (JS_GetStringBytes (JS_ValueToString (context, *vp))));
