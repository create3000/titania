...
JSBool
jsMFBool::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
	...
	else
	{
		...
		for (MFBool::size_type i = 0; i < argc; ++ i)
		{
			JSBool number;
		...
		JS_SetPrivate (context, obj, new MFBool (values, argc));
...
JSBool
jsMFBool::enumerate (JSContext* context, JSObject* obj, JSIterateOp enum_op, jsval* statep, jsid* idp)
{
	MFBool* x3darrayfield = (MFBool*) JS_GetPrivate (context, obj);
...
JSBool
jsMFBool::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
...
JSBool
jsMFBool::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
