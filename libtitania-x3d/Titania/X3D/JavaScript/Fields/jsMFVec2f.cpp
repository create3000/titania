...
JSBool
jsMFVec2f::construct (JSContext* context, JSObject* obj, uintN argc, jsval* argv, jsval* rval)
{
	if (argc == 0)
	{
	...
	else
	{
		...
		for (MFVec2f::size_type i = 0; i < argc; ++ i)
		{
			JSObject* obj2;
		...
		JS_SetPrivate (context, obj, new MFVec2f (values, argc));
...
JSBool
jsMFVec2f::get1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
...
JSBool
jsMFVec2f::set1Value (JSContext* context, JSObject* obj, jsval id, jsval* vp)
{
	if (not JSVAL_IS_INT (id))
		return JS_PropertyStub (context, obj, id, vp);
