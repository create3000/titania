...
RenderingProperties::build ()
{
	...
	try
	{
		...
		string .emplace_back (basic::sprintf (_ ("Memory usage:              %s"), strfsize (getMemoryUsage ()) .c_str ()));
		string .emplace_back ();
		string .emplace_back (basic::sprintf (_ ("Current Speed:             %.2f m/s"), getBrowser () -> getCurrentSpeed ()));
		...
		string .emplace_back (basic::sprintf (_ ("Sensors:                   %zd"), getBrowser () -> sensors () .getRequesters () .size () + getBrowser () -> prepareEvents () .getRequesters () .size ()));
	}
