...
public:
	...
	virtual
	~X3DFieldAdjustment2 () final override
	{ dispose (); }
...
template <class Type>
void
X3DFieldAdjustment2 <Type>::set_scene ()
{
	// Connect units.

	if (scene)
		scene -> units_changed () .removeInterest (&X3DFieldAdjustment2 <Type>::set_field, this);

	scene = getCurrentScene ();

	scene -> units_changed () .addInterest (&X3DFieldAdjustment2 <Type>::set_field, this);
}
...
X3DFieldAdjustment2 <Type>::setNodes (const X3D::MFNode & value)
{
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .removeInterest (&X3DFieldAdjustment2::set_field, this);
		}
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .addInterest (&X3DFieldAdjustment2::set_field, this);
		}
		catch (const X3D::X3DError &)
		{ }
	}
...
X3DFieldAdjustment2 <Type>::on_value_changed (const int index0)
{
	...
	for (const auto & node : nodes)
	{
		try
		{
			...
			field .removeInterest (&X3DFieldAdjustment2::set_field, this);
			field .addInterest (&X3DFieldAdjustment2::connect, this);
...
{
	field .removeInterest (&X3DFieldAdjustment2::connect, this);
	field .addInterest (&X3DFieldAdjustment2::set_field, this);
}
