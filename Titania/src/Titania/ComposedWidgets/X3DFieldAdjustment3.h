...
public:
	...
	X3DFieldAdjustment3 (X3DBaseInterface* const,
	...
	virtual
	~X3DFieldAdjustment3 () final override
	{ dispose (); }
...
template <class Type>
X3DFieldAdjustment3 <Type>::X3DFieldAdjustment3 (X3DBaseInterface* const editor,
               ...
               empty ()
{
	...
	buffer               .addInterest (&X3DFieldAdjustment3::set_buffer, this);
	getCurrentContext () .addInterest (&X3DFieldAdjustment3::set_field, this);
	...
	getCurrentScene () .addInterest (&X3DFieldAdjustment3 <Type>::set_scene, this);
...
template <class Type>
void
X3DFieldAdjustment3 <Type>::set_scene ()
{
	// Connect units.

	if (scene)
		scene -> units_changed () .removeInterest (&X3DFieldAdjustment3 <Type>::set_field, this);

	scene = getCurrentScene ();

	scene -> units_changed () .addInterest (&X3DFieldAdjustment3 <Type>::set_field, this);
}
...
X3DFieldAdjustment3 <Type>::setNodes (const X3D::MFNode & value)
{
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .removeInterest (&X3DFieldAdjustment3::set_field, this);
		}
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .addInterest (&X3DFieldAdjustment3::set_field, this);
		}
		catch (const X3D::X3DError &)
		{ }
	}
...
X3DFieldAdjustment3 <Type>::on_value_changed (const int index0)
{
	...
	for (const auto & node : nodes)
	{
		try
		{
			...
			field .removeInterest (&X3DFieldAdjustment3::set_field, this);
			field .addInterest (&X3DFieldAdjustment3::connect, this);
			...
			if (uniform)
			{
				...
				const auto index1  = (index0 + 1) % 3;
				const auto index2  = (index0 + 2) % 3;
...
template <class Type>
void
X3DFieldAdjustment3 <Type>::set_field ()
{
	buffer .addEvent ();
}
...
template <class Type>
void
X3DFieldAdjustment3 <Type>::connect (const Type & field)
{
	field .removeInterest (&X3DFieldAdjustment3::connect, this);
	field .addInterest (&X3DFieldAdjustment3::set_field, this);
}
