...
X3DFieldToggleButton <Type, ToggleButtonType>::setNodes (const X3D::MFNode & value)
{
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .removeInterest (&X3DFieldToggleButton::set_field, this);
		}
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .addInterest (&X3DFieldToggleButton::set_field, this);
		}
		catch (const X3D::X3DError &)
		{ }
	}
