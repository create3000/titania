...
X3DFieldAdjustment <Type>::setNodes (const X3D::MFNode & value)
{
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .removeInterest (&X3DFieldAdjustment::set_field, this);
		}
	...
	for (const auto & node : nodes)
	{
		try
		{
			node -> template getField <Type> (name) .addInterest (&X3DFieldAdjustment::set_field, this);
		}
		catch (const X3D::X3DError &)
		{ }
	}
