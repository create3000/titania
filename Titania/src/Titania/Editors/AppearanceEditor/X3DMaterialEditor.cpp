...
X3DMaterialEditor::on_material_changed ()
{
	...
	if (isTwoSidedMaterial)
		getMaterialUnlinkButton () .set_sensitive (getMaterialComboBoxText () .get_active () and twoSidedMaterial -> getCloneCount () > 1);
	else
		getMaterialUnlinkButton () .set_sensitive (getMaterialComboBoxText () .get_active () and material -> getCloneCount () > 1);
