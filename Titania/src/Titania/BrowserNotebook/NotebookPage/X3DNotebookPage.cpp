...
X3DNotebookPage::updateTitle ()
{
	...
	{
		const auto first = title +
		                    (protoPath .empty () ? "" : " • " + basic::join (protoPath .begin (), protoPath .end (), ".")) +
		                    " • " +
		                    mainBrowser -> getName ();

		const auto second = mainBrowser -> getExecutionContext () -> getWorldURL () .filename () .escape (" ") .str ();

		// Header bar

		auto & headerBar = getBrowserWindow () -> getHeaderBar ();

		headerBar .set_title (first);
		headerBar .set_subtitle (second);

		// Window title

		auto & window = getBrowserWindow () -> getWindow ();

		window .set_title (first + " · " + second);
	}
}
