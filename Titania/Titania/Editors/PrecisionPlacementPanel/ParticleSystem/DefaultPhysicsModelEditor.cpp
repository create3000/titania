...
void
DefaultPhysicsModelEditor::initialize ()
{
	X3DDefaultPhysicsModelEditorInterface::initialize ();
	X3DParticlePhysicsModelNodeInterface::initialize ();
}

void
DefaultPhysicsModelEditor::setNodes (const X3D::MFNode & value)
{
	...
	{
		if (value .back ())
			getExpander () .set_label (refineName (value .back () -> getTypeName ()));
		else
			getExpander () .set_label ("NULL");
	}
}

DefaultPhysicsModelEditor::~DefaultPhysicsModelEditor ()
{
	dispose ();
}
