...
BrowserWindow::initialize ()
{
	...
	getGridTool ()            -> getEnabled () .addInterest (&BrowserWindow::set_grid_visible,             this);
...
BrowserWindow::set_grid_visible ()
{
	...
	const auto visible = getGridTool () -> getEnabled ();

	if (visible)
	{
		getAngleGridTool ()       -> setEnabled (false);
		getAxonometricGridTool () -> setEnabled (false);
	}

	getGridLayoutToolAction () -> set_active (visible);
...
BrowserWindow::set_angle_grid_visible ()
{
	...
	const auto visible = getAngleGridTool () -> getEnabled ();
	...
	getAngleLayoutToolAction () -> set_active (visible);
...
BrowserWindow::set_axonometric_grid_visible ()
{
	...
	const auto visible = getAxonometricGridTool () -> getEnabled ();
	...
	getAxonometricLayoutToolAction () -> set_active (visible);
...
BrowserWindow::on_angle_layout_tool_toggled ()
{
	...
	getAngleGridTool () -> setEnabled (getAngleLayoutToolAction () -> get_active ());
...
BrowserWindow::on_axonometric_layout_tool_toggled ()
{
	...
	getAxonometricGridTool () -> setEnabled (getAxonometricLayoutToolAction () -> get_active ());
...
BrowserWindow::on_scenes_activated (Gtk::Menu & menu)
{
	...
	for (const auto & page : getPages ())
	{
		...
		menuItem -> set_label ((worldURL .empty () ? _ ("New Scene") : worldURL .escape (" ") .basename ()) + (modified ? "*" : ""));
		menuItem -> set_tooltip_text (worldURL .empty () ? _ ("New Scene") : worldURL .filename () .escape (" ") .str ());
