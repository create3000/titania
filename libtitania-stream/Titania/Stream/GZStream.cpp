...
gzstreambuf::open (const std::string & name, int open_mode)
{
	...
	if ((mode & std::ios::ate)or (mode & std::ios::app)
	    or ((mode & std::ios::in) && (mode & std::ios::out)))
		return (gzstreambuf*) 0;
...
gzstreambuf::flush_buffer ()
{
	...
	if (gzwrite (file, pbase (), w) not_eq w)
		return EOF;
...
void
gzstreambase::open (const std::string & name, int open_mode)
{
