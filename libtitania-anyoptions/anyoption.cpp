...
AnyOption::alloc ()
{
	...
	if (options == NULL or optiontype == NULL or optionindex == NULL)
		return false;
	else
		mem_allocated = true;
...
AnyOption::cleanup ()
{
	...
	if (values not_eq NULL)
		free (values);

	if (new_argv not_eq NULL)
		free (new_argv);
}
...
{
	if (! (valueStoreOK () and CommandSet ()))
		return;
	...
	for (int i = 1; i < argc; i ++)                  /* ignore first argv */
	{
		...
		else if (argv [i] [0] ==  opt_prefix_char)    /* POSIX char */
		{
			if (POSIX ())
			{
				...
				if (ch not_eq '0' and i < argc - 1)     /* matching char */
					setValue (ch,  argv [++ i]);
			}
...
AnyOption::matchOpt (char* opt)
{
	for (int i = 0; i < option_counter; i ++)
	{
		...
		{
			if (optiontype [i] ==  COMMON_OPT or
			    optiontype [i] ==  COMMAND_OPT)
			{ /* found option return index */
				return i;
			}
			else if (optiontype [i] == COMMON_FLAG or
...
AnyOption::matchChar (char c)
{
	for (int i = 0; i < optchar_counter; i ++)
	{
		...
		{
			if (optchartype [i] == COMMON_OPT or
			    optchartype [i] == COMMAND_OPT)
			{                                         /* an option store and stop scanning */
				return true;
			}
			else if (optchartype [i] == COMMON_FLAG or
...
AnyOption::consumeFile (char* buffer)
{
	...
	for (unsigned int i = 0; i < strlen (buffer); i ++)
	{
		...
		{
			if (pline not_eq NULL)        /* valid line */
		...
		else if (newline)                /* start of line */
		{
			...
			if ((*cursor not_eq comment)) /* not a comment */
			{
				pline      = cursor;
				linelength = 0;
			}
...
AnyOption::processLine (char* theline, int length)
{
	...
	if (*cursor == delimiter or * (cursor + length - 1) == delimiter)
	{
		justValue (pline);                                /* line with start/end delimiter */
	}
	else
	{
		for (int i = 1; i < length - 1 and ! found; i ++) /* delimiter */
		{
			if (*cursor == delimiter)
			{
...
AnyOption::justValue (char* type)
{
	...
	if (strlen (chomp (type)) == 1)         /* this is a char option */
	{
		for (int i = 0; i < optchar_counter; i ++)
		{
			if (optionchars [i] == type [0])  /* match */
			{
				if (optchartype [i] == COMMON_FLAG or
	...
	for (int i = 0; i < option_counter; i ++)
	{
		if (strcmp (options [i], type) == 0) /* match */
		{
			if (optiontype [i] == COMMON_FLAG or
