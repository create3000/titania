...
#ifndef __TITANIA_X3D_PEASE_BLOSSOM_EXPRESSIONS_DIVISION_EXPRESSION_H__
...
public:
	...
	DivisionExpression (ptr <pbStatement> && lhs, ptr <pbStatement> && rhs) :
		pbStatement (StatementType::DIVISION_EXPRESSION),
		         lhs (std::move (lhs)),
		         rhs (std::move (rhs))
	{ construct (); }
...
inline
ptr <pbStatement>
createDivisionExpression (ptr <pbStatement> && lhs, ptr <pbStatement> && rhs)
{
	if (lhs -> isPrimitive () and rhs -> isPrimitive ())
		return new PrimitiveExpression (DivisionExpression (std::move (lhs), std::move (rhs)) .getValue (), StatementType::NUMBER);
